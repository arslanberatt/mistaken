...
- **Mac model, chip, architecture, RAM, and macOS version:** Pending
- **Screen-recording permission states exercised and prompt timing observed:** Pending
- **Restart-required behavior observed on this macOS version:** Pending
- **Built-bundle Info.plist/entitlement finding, if any:** Pending
- **Negotiated sample rate, channel count, and buffer layout observed:** Pending
- **Block count, expected rate, windowed peak/RMS during known playback:** Pending
- **Silence-window observations:** Pending
- **Oversized-buffer and slow-sink observations with drop counts:** Pending
- **Start → Stop → Start cycle results, teardown durations, memory/thread/port counts:** Pending
- **Audio-only verification (registered outputs, non-audio counter, zero files written):** Pending
- **Error kinds exercised and their observed detail strings:** Pending

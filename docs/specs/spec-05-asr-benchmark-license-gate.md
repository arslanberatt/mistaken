...
### Remediation integration review and merge (integration owner, 2026-09-15)

- Branch `spec/05-remediation` (tip `1d52391`) was reviewed read-only against base `1c07446` and merged into `main` with `--no-ff` at **`aeba00e`**; merged `benchmarks/**` tree `9618edf`.
- Review covered three disjoint slices: documentation-contract consistency (`docs/context/**`, `docs/specs/**`), adapter/descriptor/harness code (`benchmarks/adapters/**`, `benchmarks/candidates/**`, `benchmarks/harness/**`), and evidence traceability (`benchmarks/reports/**`, `benchmarks/licenses/**`, `benchmarks/corpus/**`). No blocking finding was raised in any slice.
- Confirmed no gate threshold changed relative to `main`: MPR ≥ 0.90, false-correction ≤ 0.05, WER ≤ 0.25 overall / ≤ 0.12 fluent, zero silence hallucination, and every latency/RTF/CPU/RSS/stability/license/provenance/required-platform gate are byte-identical to the pre-remediation text. The only revised numbers are the product-owner-decided `long-turn` clip window (`LONG_TURN_MAX_MS` 120 000 → 180 000 ms, `benchmarks/harness/src/manifest.rs`) and the payload-delivery policy, both recorded as explicit decisions in `docs/context/progress-tracker.md`.
- Confirmed the four new optional `DecodingDescriptor` fields cannot have changed frozen results: `warmupSilenceMs` is read only by the sherpa-onnx adapter, `initialPrompt`/`noSpeechThold`/`suppressNst` only by the whisper-cpp adapter, and none of the five originally frozen candidate descriptors sets any of them (the new `whisper-base-en-q8-ggml` candidate is a separate descriptor, not a modification of `whisper-base-en-ggml`).
- `benchmarks/reports/approval.md` remains `BLOCKED — no candidate approved` after the merge; nothing in this integration approves a production model, and Specs 13–15 stay hard blocked.
- Post-merge verification on `main` (`aeba00e`): `npm run typecheck`, `npm run lint`, `npm run build`, `npm test` (7 files, 109 tests) pass; `cargo fmt --check`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo test` (56 passed), `cargo check` pass from `src-tauri`; `cargo fmt --check`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo test` (72 passed) pass in `benchmarks/harness`; `mistaken-bench validate-corpus` reports `PASS (122 primary clips, 29.6 minutes, 130 total manifest entries)` against the merged benchmark tree, executed from the recording worktree because it is the only checkout holding the Git-ignored `benchmarks/corpus/clips/**` audio.
- One non-blocking code observation, deliberately not changed: `benchmarks/adapters/vosk/src/wav_reader.h`'s defensive stereo downmix casts `acc / channels` to `int16_t` without a saturation clamp. It cannot overflow for two-channel input and the manifest admits mono clips only, so the measurement path is unaffected.
- One integration-owner root-file change accompanied the merge: `vitest.config.ts` now pins `include: ["src/**/*.{test,spec}.{ts,tsx}"]`. Vitest's default include pattern also collected `benchmarks/.vendor/whisper-cpp/examples/addon.node/__test__/whisper.spec.js` from the Git-ignored vendored upstream checkout this spec creates, so `npm test` failed (1 failed suite) on any machine that had built a benchmark adapter; with the pinned include the same worktree reports 7 files / 109 tests passing. This is the root-file requirement Spec 05 section 8 says must be handed to the integration owner rather than edited inside the spec branch.

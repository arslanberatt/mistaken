...
- **Worktree root / branch / base SHA / final commit SHA:** Pending
- **Changed paths:** Pending
- **Harness dependency versions and licenses:** Pending
- **Corpus clip count, total duration, per-condition counts, speaker-profile count:** Pending
- **Manifest digest used for each report:** Pending
- **`validate-corpus` command and result:** Pending
- **Runtime tags built and compiler/CMake versions per host:** Pending
- **Model files, recomputed SHA-256, byte sizes, and payload totals:** Pending
- **`mac-arm64` host identity (chip, cores, RAM, macOS version, power/thermal state):** Pending
- **`win-x64` host identity (CPU, cores, RAM, edition/version/build, power plan):** Pending
- **Per-candidate WER, MPR, false-correction rate, insertion rate, latencies, RTF, peak RSS, CPU time, per host, with repetition spread:** Pending
- **Two-stream concurrency measurements:** Pending
- **Networking-disabled verification method and result per host:** Pending
- **Gate evaluation table per candidate per host:** Pending
- **License record completeness and verdicts, including the 2023-06-26 upstream provenance resolution:** Pending
- **Approval decision or explicit blocker with failing gate:** Pending
- **Second-operator reproduction deltas:** Pending
- **`cargo fmt --check` / `clippy` / `test` results per host:** Pending
- **Privacy inspection (`git check-ignore`, staged-file list, no personal attributes):** Pending

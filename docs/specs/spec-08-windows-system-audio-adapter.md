...
- **Resolved `windows` crate version, exact features, and licenses:** Pending
- **PC model, CPU, architecture, RAM, Windows edition/version/build:** Pending
- **Default output device class and endpoint id handling (sanitized):** Pending
- **`availability()` results per induced condition:** Pending
- **Observed mix format: tag/subformat, bit depth, channels, sample rate, block align:** Pending
- **Conversion branch exercised and test results per branch:** Pending
- **Block count, expected rate, windowed peak/RMS during known playback:** Pending
- **Idle-phase counters (captured vs silent-flag vs synthesized) and resume behavior:** Pending
- **Long-idle cap and truncation observations:** Pending
- **Discontinuity and timestamp-error observations:** Pending
- **Slow-sink drop counts and memory stability:** Pending
- **Default-endpoint switch detection latency and result:** Pending
- **Device invalidation and retry observations:** Pending
- **Start → Stop → Start cycle results, teardown durations, handle/thread/working-set counts:** Pending
- **COM initialization/uninitialization thread audit:** Pending
- **Privacy-setting effect observation:** Pending
- **Stock-device-only confirmation (no Stereo Mix, cable, or driver):** Pending
- **DRM-protected content observation, if tested:** Pending
- **Offline verification method and result:** Pending
- **Crate `fmt`/`clippy`/`test`/example build results:** Pending
- **`unsafe` block inventory with documented invariants:** Pending
- **Yielded shared-file requirements with exact values:** Pending

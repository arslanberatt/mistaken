...
- **Status:** Implemented and **partially verified on real Windows hardware** (Windows 10 Education, version 10.0.19045, build 19045, x64; Intel Core i5-11400H; default render endpoint `Kulaklıklar (HyperX Cloud III Wireless)`), on 2026-09-14. Natively executed and clean: `cargo fmt --check`, `cargo check --locked --all-targets`, `cargo clippy --locked --all-targets --all-features -- -D warnings`, `cargo test --locked` (35 passed / 0 failed), and `cargo build --locked --all-targets`. Real WASAPI loopback capture of real system playback is **confirmed**: 48 kHz Float32 endpoint mix, exactly 50 blocks/s of exactly 960-sample mono blocks with zero wrong-length blocks and zero out-of-range samples, windowed RMS within 0.3 % of the played tone's theoretical value, bounded buffering proven under a refusing sink (537 counted drops with no stall and no memory growth), six consecutive start/stop cycles with flat handle/thread/working-set counts and ~10 ms teardowns, and zero sockets opened during capture. Still explicitly **BLOCKED, not passed**: criterion 16 (default-endpoint switch), criteria 9/10 (real silence-gap synthesis and gap cap), criteria 7/17 (elevated audio-service-down, no-render-endpoint, and device-invalidation paths), plus the partial gaps in criteria 4, 5, 8/12, and 19 — each with its exact reason recorded in section 16 and in `crates/windows-system-audio/platform-notes-windows.md`. Not mergeable until those BLOCKED items are closed on a suitable host and high-capability review completes.
...
Filled from an actual verification run; every field states what was executed, and every unexercised item says so explicitly rather than being reported as passed.

- **Implementation status:** Implemented; **partially verified on real Windows hardware** on 2026-09-14. The crate's full check set and real WASAPI loopback capture of real system playback were executed natively on Windows 10 build 19045. The criteria that require changing the machine's default output device, an idle audio engine, or Administrator rights are **BLOCKED, not passed** — itemized under "Not exercised" below and in `crates/windows-system-audio/platform-notes-windows.md`.
- **Canonical repository root:** `C:\Users\Berat\mistaken` on the Windows verification host, `main` at merge commit `6e52978c3476f7e6834a6ee5b038034452fcae9d` (Wave 2: Specs 01, 02, 03, 05 merged). The original implementation pass was authored on a macOS host at `/Users/berat/mistaken`; the branch was cloned from GitHub onto Windows for this verification.
- **Worktree root / branch / base SHA / implementation commit SHA:** `C:\Users\Berat\mistaken`, branch `spec/08-windows-system-audio`, base SHA `6e52978c3476f7e6834a6ee5b038034452fcae9d`. The branch carries the original implementation commit `6ed5847f8e277cfa1bf0f3c1ef451cf06ebfc108` plus one Windows-verification commit; exact hashes are retrievable via `git log --oneline spec/08-windows-system-audio` and are intentionally not hardcoded here to avoid a self-referential value that changes with every edit of this file.
- **Changed paths:** exactly `crates/windows-system-audio/**` (new: `Cargo.toml`, `Cargo.lock`, `.gitignore`, `README.md`, `platform-notes-windows.md`, `src/{lib,error,config,format,blocks,timeline,com,endpoint,capture}.rs`, `examples/system_audio_probe.rs`) plus this spec file's own sections 1 and 16. No file under `src/**`, `src-tauri/**`, `benchmarks/**`, `crates/macos-system-audio/**`, `docs/context/**`, the repository root, or another spec file was touched — confirmed by `git status`/`git diff --stat` against the base SHA. The Windows-verification pass changed only `examples/system_audio_probe.rs`, `platform-notes-windows.md`, and this spec file; **no `src/**` module was modified**, because no Windows-specific implementation defect was found.
- **Resolved `windows` crate version, exact features, and licenses:** `windows = "=0.62.2"` (Cargo.lock pins checksum `527fadee13e0c05939a6a05d5bd6eec6cd2e3dbd648b9f8e447c6518133d8580`), `default-features = false`, features `Win32_Foundation`, `Win32_Media_Audio`, `Win32_Security`, `Win32_System_Com`, `Win32_System_Com_StructuredStorage`, `Win32_System_SystemInformation`, `Win32_System_Threading`, `Win32_System_Variant`, `Wdk_System_SystemServices`. License: MIT OR Apache-2.0 (Microsoft). **Confirmed on Windows:** every check ran with `--locked` and resolved without touching the lockfile; the only crates that resolve are `windows` 0.62.2 with its `windows-*` support crates (`windows-core`, `windows-collections`, `windows-future`, `windows-implement`, `windows-interface`, `windows-link`, `windows-numerics`, `windows-result`, `windows-strings`, `windows-threading`) plus the ordinary proc-macro build crates `proc-macro2`, `quote`, `syn`, `unicode-ident`. No `cpal`, no third-party audio crate, no HTTP client, no telemetry dependency.
- **PC model, CPU, architecture, RAM, Windows edition/version/build:** MONSTER ABRA A5 V17.2; 11th Gen Intel Core i5-11400H @ 2.70 GHz, 6 cores / 12 logical; x64; 15.8 GB RAM; **Microsoft Windows 10 Education, version 10.0.19045, build 19045, 64-bit** — exactly the crate's documented tested floor. Toolchain: rustc 1.98.1 (48a229cea 2026-09-01) / cargo 1.98.1, host tuple `x86_64-pc-windows-gnu`, linking through MinGW-w64 gcc 15.2.0 (WinLibs UCRT, POSIX threads, SEH). **Target caveat:** no MSVC toolchain or Windows SDK exists on this host, so the crate has still never been built or run for `x86_64-pc-windows-msvc`; Spec 09/14 must repeat the check set on the MSVC target it ships.
- **Default output device class and endpoint id handling (sanitized):** `GetDefaultAudioEndpoint(eRender, eConsole)` resolved to `Kulaklıklar (HyperX Cloud III Wireless)` — a USB wireless headset render endpoint — id `{0.0.0.00000000}.{0073df4e-7175-4db4-8ef3-e890953f78f3}`, state ACTIVE. Handling confirmed by code and by output: the crate reads the id solely to compare against a fresh once-per-second lookup and never logs, prints, or exposes it; the id recorded here came from a separate enumeration helper, not from this crate.
- **`availability()` results per induced condition:** Success path **exercised** — `availability(): Ok` with a default output device present, on every one of the runs below, starting no thread and capturing nothing. The `NoRenderEndpoint` and `AudioServiceDown` conditions are **BLOCKED**: inducing them requires disabling every render endpoint or stopping the `Audiosrv` service, both of which need Administrator rights that this session did not have.
- **Observed mix format: tag/subformat, bit depth, channels, sample rate, block align:** Partially observed. **Sample rate 48 000 Hz** and **conversion branch `Float32`** were reported by the probe from a real `IAudioClient::GetMixFormat` result, with zero format rejections across every run. **Not recorded:** the raw `wFormatTag`/`WAVEFORMATEXTENSIBLE` subformat, bit depth, block align, and the endpoint's *source* channel count — no public API surfaces them. `SystemAudioFormat.channels` is hardcoded to `1` and is the delivered mono count, not the endpoint's, so the probe's `channels=1` must not be read as "the endpoint is mono". `ValidatedFormat.source_channels` exists and drives the downmix internally but is not public.
- **Conversion branch exercised and test results per branch:** All 35 unit tests now run **natively on Windows** as the crate's own tests (previously executed only in a throwaway scratch crate on macOS), 35 passed / 0 failed / 0 ignored. Float32: `float32_mono_round_trips_without_downmix`, `float32_stereo_downmixes_to_mean`, `every_delivered_sample_is_finite_and_in_range` — pass. PCM16: `pcm16_full_scale_values_convert_and_clamp` — pass. PCM24: `pcm24_sign_extends_negative_values`, `pcm24_max_positive_value_is_near_one` — pass. PCM32: `pcm32_full_scale_values_convert_and_clamp` — pass. Extensible subformat discrimination, channel/rate/block-align rejection, unsupported-tag and unsupported-bit-depth rejection all pass. The **real endpoint exercised the Float32 branch only**; the integer-PCM branches remain test-only, since the endpoint mix format is not selectable.
- **Block count, expected rate, windowed peak/RMS during known playback:** Known playback was a generated 440 Hz sine at amplitude 0.5, 48 kHz 16-bit stereo, played through the default endpoint. Block cadence was **exactly 50 blocks per second** in every reporting window of every run (24 s, 18 s, 12 s, 10 s, and 6 × 4 s). Every block was **exactly 960 mono samples** (`block_len_min = block_len_max = 960 = 48000 / 50`), with `wrong_length_blocks = 0` and `out_of_range_samples = 0` over more than 5 000 blocks — every delivered sample finite and inside `[-1.0, 1.0]`, and partial tails discarded rather than padded. Windowed peak ranged 0.5048 – 0.7734 and windowed RMS 0.3535 – 0.3556 against the tone's theoretical 0.5 / sqrt(2) = 0.35355, i.e. within 0.3 %. The windowed peak exceeding the played amplitude is the endpoint's own DSP in the session mix, not a conversion defect — a gain or conversion error could not have left RMS that accurate.
- **Idle-phase counters (captured vs silent-flag vs synthesized) and resume behavior:** **BLOCKED on real hardware.** With the tone stopped, windowed peak fell to 0.018 – 0.056 and windowed RMS to 0.0015 – 0.0058 within one second, and recovered to 0.52+ within one second of resuming playback with no timeline jump — so the *content* transition was observed. But `synthesized_frames` stayed **0** throughout: the audio engine never went idle because Discord (pid 7288) and Discovery-d (pid 23236) held continuous render streams on the default endpoint, so WASAPI kept delivering real packets and the capture loop's `WAIT_TIMEOUT` synthesis path was never entered. What was observed is nonetheless honest — those near-silent frames were genuinely captured and were counted as `captured_frames`, never presented as synthesized. The synthesis path remains covered only by unit tests `idle_gap_between_packets_is_synthesized_and_counted` and `idle_wait_timeout_synthesizes_and_advances_expected_position`.
- **Long-idle cap and truncation observations:** **BLOCKED on real hardware** for the same reason — `truncated_gap_events` stayed 0 because no gap ever occurred. Proven at the algorithm level by `gap_longer_than_cap_is_truncated_and_resynchronized` and `idle_wait_timeout_respects_the_cap`, which now run natively on Windows: a gap or idle wait beyond `max_gap_fill_frames` emits exactly the cap, increments `truncated_gap_events` by exactly one, and resynchronizes so the next packet sees no residual gap.
- **Discontinuity and timestamp-error observations:** Discontinuity **observed on real hardware**: `discontinuity_events` incremented to exactly 1 on the first packet after `IAudioClient::Start()` in every session and never again, and no silence was synthesized for that apparent position jump — the real-packet behavior matches `discontinuity_is_counted_and_skips_gap_synthesis`. `AUDCLNT_BUFFERFLAGS_TIMESTAMP_ERROR` and `AUDCLNT_BUFFERFLAGS_SILENT` were **never raised by a real packet** on this host (`silent_flag_frames` stayed 0), so those two flag paths remain covered only by their unit tests.
- **Slow-sink drop counts and memory stability:** **Exercised on real hardware.** A bounded sink (capacity 4 blocks) with a consumer draining one block every 200 ms — 5 blocks/s consumed against 50 blocks/s produced — over 12 s produced `dropped_blocks` of 41, 86, 131, 177, 222, 267, 312, 357, 402, 447, 492, **537**: about 45 drops per second, exactly the production/consumption difference. Block production held at **exactly 50 blocks/s in all 12 windows**, so the refusing sink never stalled the capture loop; working set moved only 8.87 → 8.92 MB, so nothing was queued internally; and no counter other than `dropped_blocks` was affected. No retry occurred.
- **Default-endpoint switch detection latency and result:** **BLOCKED — not exercised.** Measuring it requires changing the machine's default render endpoint mid-capture. The host had live voice-call render streams open, and moving the default output would have disrupted a real user session, so the switch was not performed. The once-per-second `has_default_endpoint_changed` poll ran throughout every session and correctly reported no change, but the positive `EndpointChanged` branch and its latency are unmeasured.
- **Device invalidation and retry observations:** **BLOCKED — not exercised.** Inducing `AUDCLNT_E_DEVICE_INVALIDATED` requires disabling the active render endpoint (Administrator rights, not held) or physically powering off / unplugging the wireless headset's dongle. Neither the `DeviceInvalidated` mapping nor the retry-on-new-default path was exercised against a live stream.
- **Start → Stop → Start cycle results, teardown durations, handle/thread/working-set counts:** **Exercised on real hardware — six consecutive cycles**, 4 s of capture each, capture confirmed working in every cycle. `start()` took 15.6 / 9.6 / 6.5 / 5.6 / 6.5 / 7.1 ms; `stop()` took 10.3 / 10.4 / 10.6 / 10.5 / 10.4 / 10.5 ms, all far inside the one-second budget; a second `stop()` on each session returned Ok as a no-op, confirming idempotent teardown. Sampled twice per second: **threads** 5 while capturing, 4 between cycles, back to 5 on the next start — the one capture thread appears and disappears with the session and never accumulates; **handles** 185 → 186 between cycles 1 and 2 then flat at 186 for cycles 2 – 6 (163/164 between cycles), a one-time lazy COM/CRT initialization rather than a per-cycle leak, which would have shown six increments; **working set** 8.81 → 9.03 MB across 26 s and six cycles. A separate 24 s single session held threads at 5, handles at 182, and working set at 8.84 → 8.90 MB start to finish. **CPU:** 0.11 s of processor time over 26 s wall clock (~0.4 % of one core), and 0.25 s over the 12 s bounded-sink run.
- **COM initialization/uninitialization thread audit:** By construction and code review: `ComScope::initialize_mta()` is called only from inside the closure passed to `std::thread::Builder::spawn` in `capture.rs::start()` (via `initialize_session`, itself only called from `run_capture_thread`) and from `availability()`'s own synchronous body (which runs on the caller's thread but never spawns a thread or starts capture — it only enumerates and inspects). No call site initializes or uninitializes COM anywhere else. **Supported on real hardware** by the six-cycle handle/thread evidence above: a missing `CoUninitialize` or a leaked interface across six full cycles would have shown as monotonically rising handle counts, and handles were flat from cycle 2 onward. Dedicated COM leak tooling was not run.
- **Privacy-setting effect observation:** Partially observed. Both `HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\microphone` and the matching `HKCU` key read `Allow` throughout every run, and loopback capture succeeded with **no prompt of any kind at any point** — consistent with the spec's no-permission-theater claim. The negative direction (flipping the toggle to `Deny` and confirming loopback is unaffected) was **not exercised**, so the crate claims only that loopback works with the toggle allowed, not that the toggle is proven irrelevant.
- **Stock-device-only confirmation (no Stereo Mix, cable, or driver):** **Confirmed on real hardware.** The host's audio endpoint inventory, enumerated before and after the capture runs, was unchanged: three active render endpoints (`Kulaklıklar (HyperX Cloud III Wireless)`, `LS27AG32x (HD Audio Driver for Display Audio)`, `Hoparlör (Realtek(R) Audio)`) and two active capture endpoints (`Mikrofon (HyperX Cloud III Wireless)`, `Mikrofon (Realtek(R) Audio)`). **No "Stereo Mix" endpoint exists on this host at all**, no virtual audio cable is installed, and nothing was installed or enabled as a side effect of this crate. Capture worked against the stock default output device alone.
- **DRM-protected content observation, if tested:** **Not tested.** No protected stream was played against this crate; the DRM limitation in the README and platform notes is recorded from Microsoft's documentation, not from observation.
- **Offline verification method and result:** **Exercised.** While a full capture session ran, the probe process owned **zero TCP connections and zero UDP endpoints** for its entire lifetime, polled continuously against its pid with `Get-NetTCPConnection` / `Get-NetUDPEndpoint`. Combined with the `--locked` dependency set (no HTTP, DNS, socket, or telemetry crate resolves), this is direct runtime evidence of no network path. No audio file was created and no sample value was printed — the probe emits aggregate statistics only. The network adapter itself was not disabled, because doing so requires Administrator rights; the zero-socket measurement is the stronger evidence in any case, since it proves the process opened nothing rather than merely that nothing could connect.
- **Crate `fmt`/`clippy`/`test`/example build results:** All **native on Windows**, all exit 0: `cargo fmt --check` clean; `cargo check --locked --all-targets` clean; `cargo clippy --locked --all-targets --all-features -- -D warnings` clean with zero warnings; `cargo test --locked` **35 passed, 0 failed, 0 ignored**; `cargo build --locked --all-targets` clean, producing the `system_audio_probe` example binary that all the real-hardware evidence above was gathered with. This supersedes the earlier macOS-only cross-compilation `check`/`clippy` result.
- **`unsafe` block inventory with documented invariants:** 32 `unsafe` blocks/`unsafe fn`s across `capture.rs` (28), `com.rs` (2), and `endpoint.rs` (2, one of which wraps three FFI calls: `GetId`, `to_string`, `CoTaskMemFree`). Every one carries a preceding `// Safety:` comment (or, for `unsafe fn`s, a `# Safety` doc-comment section) stating the invariant relied upon: pointer/handle validity and lifetime for the duration of the call, `GetBuffer`/`ReleaseBuffer` whole-packet-and-immediate-release pairing, the packed-struct `read_unaligned` copy-before-compare rule for `WAVEFORMATEXTENSIBLE`, and the COM-scope-owns-its-own-thread rule. No `unsafe` block lacks a stated invariant. Unchanged by the Windows-verification pass, which modified no `src/**` file.
- **Yielded shared-file requirements with exact values:** (1) Windows minimum-version declaration: API floor Windows 10 build 15063, supported/tested floor Windows 10 22H2 (build 19045)/Windows 11 — now confirmed to run correctly on build 19045 — for the shared application configuration Spec 09 owns. (2) Registration of `windows-system-audio` as a Windows-only dependency and wiring into the runtime capture session — Spec 09's work; this crate exposes `availability()`/`start()`/`SystemAudioSession`/`SystemAudioSink` plus the mapping table in section 6 for that wiring. (3) Packaging/runtime note for Spec 14: this crate adds no redistributable, no driver, and no extra capability — confirmed by its dependency list and by the unchanged device inventory above. (4) **New:** Spec 09/14 must run this crate's check set once on the `x86_64-pc-windows-msvc` target, which has never been built here.
- **Temporary artifact cleanup:** The Windows-verification pass used only scratch files outside the repository (a generated test-tone WAV, endpoint/session enumeration helpers, and scenario driver scripts), all under the session scratchpad and none committed. `crates/windows-system-audio/target/` is ignored by the crate's `.gitignore`. No debug logging, allocation/timing instrumentation, experimental render-stream pacing code, `AUTOCONVERTPCM` experiment, per-process loopback attempt, or `IMMNotificationClient` prototype exists in the committed source — the crate implements exactly the frozen design in section 6.
- **High-capability review findings and dispositions:** Self-reviewed against sections 11/12/15 during implementation and again during Windows verification (resource inventory, COM/handle release order, packet-protocol alternation, timeline honesty, format conversion, boundedness, endpoint-change detection). **No Windows-specific implementation defect was found**: no `src/**` file needed changing, and every check and capture scenario passed on first execution. Two *evidence-surface* gaps were found and are recorded rather than silently fixed: the probe reported only cumulative peak/RMS (now also reports per-window statistics, block-length validation, cycle timing, and a bounded-sink mode), and the public `SystemAudioFormat` does not expose the endpoint's source channel count or raw format tag, which limits criteria 8/12 evidence. A mandatory independent high-capability review per `spec-plan.md`'s review gates, and the BLOCKED real-hardware items below, remain outstanding before this branch may be considered mergeable.
- **Not exercised — BLOCKED (must not be reported as passed):** criterion 16 (default-endpoint switch mid-capture: would have disrupted live voice-call audio on the host); criteria 9 and 10 (real silence-gap synthesis and gap cap: the audio engine never idled because Discord and Discovery-d held continuous render streams); criteria 7 and 17 (`AudioServiceDown`, `NoRenderEndpoint`, and `DeviceInvalidated`: all require Administrator rights, which the verification session did not hold, or physical access to the wireless endpoint); criterion 5 in part (the microphone privacy toggle's `Deny` direction); criterion 4 in part (the sub-15063 `Unsupported` branch cannot be induced on a build-19045 host); criteria 8 and 12 in part (the endpoint's source channel count, bit depth, block align, and raw format tag/subformat are not surfaced by any public API); criterion 19 in part (no DRM-protected content played). Full reasons in `crates/windows-system-audio/platform-notes-windows.md`, section "Not exercised — BLOCKED".
- **Final Git status:** `git status --porcelain` is clean immediately after the Windows-verification commit; the branch diff against base SHA `6e52978c3476f7e6834a6ee5b038034452fcae9d` touches exactly the 15 files under `crates/windows-system-audio/**` plus this spec file — confirmed directly, not asserted.
